...
class PatientHealthEntry extends StatefulWidget {
  const PatientHealthEntry({super.key});

  @override
  State<PatientHealthEntry> createState() => _PatientHealthEntryState();
}

class _PatientHealthEntryState extends State<PatientHealthEntry> {
  List availablePorts = [];
...
//////////////////////////////////////////////////////////////////////////////////////
  ...
  void sendDataHealthrecord() async {
    ...
    var res = await http.post(
      ...
      body: {
        ...
        "dtx": context.read<DataProvider>().bloodGlucoseHealthrecord.text,
        "bp":
  ...
  void sendHealthrecordGateway() async {
    ...
    var body = jsonEncode({
      ...
      "dtx": provider.bloodGlucoseHealthrecord.text,
    });
    try {
      provider.debugPrintV(
  ...
  void onValueChanged(Map<String, dynamic> params) {
    ...
    } else if (params['type'] == 'dtx') {
      setState(() {
        context.read<DataProvider>().bloodGlucoseHealthrecord.text =
            params['dtx'].toString();
      });
    } else if (params['type'] == 'status') {
      print('status ${params['name']} ${params['status']}');
    } else if (params['type'] == 'hl7') {
      print('hl7 ${params['name']} ${params['status']}');
      setState(() {
        if (params['pr'] != null) {
          context.read<DataProvider>().pulseHealthrecord.text = params['pr']
              .toString();
        ...
        if (params['sys'] != null) {
          context.read<DataProvider>().sysHealthrecord.text = params['sys']
              .toString();
        }
        if (params['dia'] != null) {
          context.read<DataProvider>().diaHealthrecord.text = params['dia']
              .toString();
        }
        if (params['spo2'] != null) {
          context.read<DataProvider>().spo2Healthrecord.text = params['spo2']
        ...
        if (params['temp'] != null) {
          context.read<DataProvider>().tempHealthrecord.text = params['temp']
              .toString();
        }
      });
    }
  ...
  void initState() {
    ...
    context.read<DataProvider>().bloodGlucoseHealthrecord =
  ...
  void dispose() async {
    ...
    _deviceManager.setOnValueChanged(null);
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        BoxRecord(
                          image: 'assets/kauo.png',
                          texthead: 'DTX',
                          keyvavlue: provider.bloodGlucoseHealthrecord,
                        ),
                      ],
                    ),
                  ),
